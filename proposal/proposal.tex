...
For the benchmarking and the auto-scaling, we will use a simple web-server scenario. There are two types of machines, load-balancers and worker machines. The load-balancers get an ip from a predefined pool of elastic-ip's, such that requests from clients can be sent to them. All load-balancer connect to the overlay network and receive the list of worker machines from the coordinator. Incoming request are sent to a worker machine using a load-balancing technique. Therefore load-balancers are connected to all machines in the network, where as worker machines are only connected to the load-balancers.
The worker machines process the request and contain no further application logic, they aren't members of the overlay network.
The members of the overlay network (load-balancers) elect a coordinator and perform a re-electing in case the coordinator dies.
...
The auto-scaling is performed by the coordinator. If the load on the worker machines is too large, the coordinator will scale-up some machines or start new machines (scale-out). The load-balancers can be scaled-up and scaled-out as well, if they cannot handle the submitted workload. For the scale-out there will be a snapshot with the installed application to improve the start-up time. 
